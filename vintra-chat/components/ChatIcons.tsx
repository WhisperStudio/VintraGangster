...
import styles from "./ChatIcons.module.css";
...
export type IconType =
  ...
  | "sleep";
/**
 * AnimatedInactiveIcon:
 * Starter i “inactive” (med blinking),
 * etter 5–10s bytter til “heart” (én animasjon på 8s),
 * så tilbake til “inactive” og gjentar.
 */
export const InactiveIcon: React.FC = () => {
  const [mode, setMode] = useState<"inactive" | "heart">("inactive");
  const heartTimer  = useRef<number|undefined>(undefined);
  const returnTimer = useRef<number|undefined>(undefined);

  // planlegg første “heart”-syklus
  useEffect(() => {
    scheduleHeart();
    return () => {
  ...
  function scheduleHeart() {
    // vent minst 5s + opptil 5s tilfeldig
    const delay = 5000 + Math.random() * 5000;
    heartTimer.current = window.setTimeout(() => {
      setMode("heart");
      // kjør “heart” i 8s før vi går tilbake
  ...
  if (mode === "inactive") {
    // —— eksisterende SVG med blinking-øyne ——
    return (
      ...
      <rect
        ...
        stroke-width="8"
      />
      <circle className={styles.eye} cx="33" cy="40" r="8" />
      
      <line className={styles["eye-line"]}  x1="24" y1="40" x2="40" y2="40" />

      <circle className={styles.eye}  cx="67" cy="40" r="8" />
      
      <line className={styles["eye-line"]}  x1="60" y1="40" x2="74" y2="40" />
    ...
    return (
      <svg viewBox="0 0 100 100" className={styles.icon}>
        ...
        />
        {/* “forgrunnsøye” */}
        <circle className={styles.eye_heart} cx="33" cy="40" r="8" fill="#ffffff" />
        <circle className={styles.eye_heart} cx="67" cy="40" r="8" fill="#ffffff" />

        {/* venstre hjerte */}
        <g className={styles["heart-group"]}>
          <path
            d="M26,37.827
                c0-1.025,0.365-2.114,1.002-2.988
                c0.729-1,1.684-1.551,2.691-1.551
                c1,0,1.942,0.437,2.625,1.173
                c0.684-0.736,1.626-1.173,2.626-1.173
                c1.006,0,1.961,0.551,2.688,1.551
                c0.637,0.874,1.002,1.963,1.002,2.988
                c0,1.628-1.006,3.473-2.991,5.485
                c-1.414,1.434-2.812,2.425-2.87,2.466
                c-0.137,0.097-0.296,0.145-0.455,0.145
                c-0.156,0-0.313-0.046-0.446-0.138
                C31.632,45.62,26,41.717,26,37.827z"
            fill="#ffffff"
            />
        </g>
        ...
        <g className={styles["heart-group"]}>
          <path
            d="M60,37.827
                ...
                c-0.137,0.097-0.296,0.145-0.455,0.145
                c-0.156,0-0.313-0.046-0.446-0.138
                C65.632,45.62,60,41.717,60,37.827z"
            fill="#ffffff"
            />
        </g>
...
export const TiredIcon: React.FC = () => ( 
...
export const SleepIcon: React.FC = () => {
  ...
  const activeZsRef = useRef<number>(0);
  const maxZsRef = useRef<number>(3); // Maksimalt 3 Z-er samtidig

  useEffect(() => {
    const svg = svgRef.current;
    if (!svg) return;

    // Lager en "Z" som SVG-text element
    function createZ() {
      if (!svg || activeZsRef.current >= maxZsRef.current) return;
      
      activeZsRef.current++;
      
      const z = document.createElementNS('http://www.w3.org/2000/svg', 'text');
      z.textContent = 'Z';
      z.setAttribute('class', styles['z-letter']);
      
      // Tilfeldig størrelse
      const sizes = ['14', '16', '18'];
      const fontSize = sizes[Math.floor(Math.random() * sizes.length)];
      z.setAttribute('font-size', fontSize);
      z.setAttribute('font-weight', 'bold');
      z.setAttribute('fill', '#ffffff');
      z.setAttribute('opacity', '0');
      z.setAttribute('text-anchor', 'middle');
      
      // Posisjon ved munnen (SVG viewBox: 0-103.696)
      const baseX = 52 + (Math.random() - 0.5) * 8; // Mindre variasjon i startposisjon
      const baseY = 75; // Ved munnen
      
      z.setAttribute('x', baseX.toString());
      z.setAttribute('y', baseY.toString());
      
      svg.appendChild(z);
      
      // Start animasjonen med CSS
      requestAnimationFrame(() => {
        z.classList.add(styles.floating);
      });
      
      // Fjern elementet etter animasjonen og reduser teller
      setTimeout(() => {
        if (z.parentNode) {
    ...
    // Spawn Z-er hver 0.8 sekund for flere samtidige Z-er
