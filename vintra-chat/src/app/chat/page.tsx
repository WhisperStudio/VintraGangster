    ...
    <>
      ...
      <div className="fixed bottom-4 right-4 z-50">
        ...
        <div className={`mt-2 w-80 h-[520px] bg-[#0a0a0a]
          ...
          >
            {messages.map((m,i) => (
              <div key={i} className={`flex ${m.sender==="user"?"justify-end":"justify-start"}`}>
                <div className={`p-2 rounded-lg max-w-xs ${m.sender==="user"?"bg-white text-black":"bg-gray-800"}`}>
                  {m.text}
                </div>
              </div>
            ))}
          ...
          <form onSubmit={handleSubmit} className="p-3 bg-black/20 border-t border-gray-700 flex gap-2">
            <input
              type="text"
              value={input}
            ...
            />
            <button type="submit" className="bg-white text-black px-3 rounded-lg">
              Send
            </button>
          </form>
        </div>
