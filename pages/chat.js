...
export default function ChatPage() {
  ...
  const addMessage = (text, sender) => {
    setMessages((m) => [...m, { text, sender }]);
  };

  const handleBubbleClick = () => {
    setShowWindow((v) => !v);
    if (!showWindow && messages.length === 0) {
      addMessage(
        "Hei! Jeg er Vintra sin AI-assistent. Hva kan jeg hjelpe deg med?",
        "bot"
      );
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setIsTyping(false);
      addMessage(reply ?? "Beklager, ingen svar mottatt.", "bot");
    } catch (err) {
      setIsTyping(false);
  ...
  return (
    <>
      ...
      <div id="chat-widget-container" className="fixed bottom-4 right-4 z-50">
        {/* Chat window */}
        <div
          id="chat-window"
        ...
        >
          <div className="p-4 bg-black/30 border-b border-gray-800 flex items-center space-x-3">
          ...
          >
            ...
            {isTyping && (
              <div className="typing-indicator flex items-center gap-2 pl-2">
        ...
        >
          <div
            id="chat-label"
            className={`curved-text absolute -top-5 left-1/2 -translate-x-1/2 transition-opacity ${
              showWindow ? "opacity-0" : "opacity-100"
            }`}
          >
            <svg viewBox="0 0 200 100" className="w-44 h-16">
              <defs>
                <path
                  id="arcPath"
                  d="M 40 50 A 60 60 0 0 1 160 50"
                  fill="none"
                />
              </defs>
              <text>
